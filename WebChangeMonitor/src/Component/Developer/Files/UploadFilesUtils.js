...
export function getModifiedFiles(filesArray, serverFiles, newVersionFiles) {
	return new Promise((resolve, reject) => {
		...
		filesArray.forEach((element, index) => {
			...
			let serverFileIndex = serverFiles.findIndex((file) => file.file.localRelativePath === element.file.webkitRelativePath);
			// console.log(
			...
			if (serverFileIndex !== -1) {
				...
				const myPromise = isFileModified(element.file, serverFiles[serverFileIndex], index);
		...
		Promise.all(promises).then((result) => {
			...
			result.forEach((element) => {
				if (element.isModified) {
					...
					//get index
					const index = newVersionFiles.findIndex((x) => x.encodedName === element.serverFile.encodedName);
					console.log("before item removed", [...updateVersionFiles]);
					console.log("item removed :", updateVersionFiles.splice(index, 1), index); //].statusId = 2;
...
export function getDeletedFiles(filesArray, serverFiles) {
	return new Promise((resolve, reject) => {
		...
		serverFiles.forEach((element, index) => {
			...
			let localIndex = filesArray.findIndex((item) => item.file.webkitRelativePath === element.file.localRelativePath);
			if (localIndex === -1) {
				deletedFiles.push({
					file: element,
					isDeleted: true,
