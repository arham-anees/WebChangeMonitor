...
namespace WebChangeMonitor.API.Controllers
{
  ...
  public class FilesController : ControllerBase {
    ...
    public IActionResult UploadFiles(IFormFile file) {
      ...
      Console.WriteLine("modified files sent for uploading");
      ...
      if (ModelState.IsValid) {
        try {
          ...
          //foreach (var file in files)
          {
            ...
            serverPath =Path.Combine(_WebHostEnvironment.ContentRootPath , "Resources" , encodedName);
        ...
        public IActionResult CheckFiles([FromBody]IEnumerable<CheckChangedFilesActionModel> checkChangedFiles) {
            try {
        Console.WriteLine("files sent for checking modification");
        ...
        public IActionResult GetFile(string encodedName) {
            try {
                var serverPath = _WebHostEnvironment.ContentRootPath + "/Resources/" + encodedName;
